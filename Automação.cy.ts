...
describe('Fluxo Completo', () => {
  ...
  it('User Realizando Cadastro correto e Seleção de opções', () => {
    ...
    cy.get('#formComidaCarne').check().should('be.checked').wait(1000).click();
    cy.get('#formComidaFrango').check().should('be.checked').wait(1000).click();
    cy.get('#formComidaVegetariana').check().should('be.checked').wait(1000).click();
  ...
  it('User Realizando Cadastro incorreto e corrigindo no meio do processo junto com as interações divergentes', () => {
    cy.visit('https://www.wcaquino.me/cypress/componentes.html')
    cy.get('#formNome')
      .type('Thiago Gibson Santos dos Reis')
    cy.get('#formCadastrar').click();
    cy.get('#resultado').should('contain', 'Nao cadastrado');
    cy.get('#formNome').type('{backspace}{backspace}{backspace}{backspace}{backspace}{backspace}{backspace}{backspace}{backspace}{backspace}{backspace}{backspace}{backspace}{backspace}{backspace}')
    ...
    cy.get('#formComidaVegetariana').click().should('be.not.checked')
    cy.get('#formEscolaridade').select('Mestrado')
  ...
  it('Interações de tela com mudanças apenas nos botões', () => {
    ...
    cy.get('#buttonLazy').click().should('have.value', 'zZz ZzZ!')
    cy.get('#buttonCount').click().should('have.value', '11')
    cy.get('#buttonCount').then(($btn) => {
      for (let i = 0; i < 5; i++) {
        cy.wrap($btn).click();
      }
      cy.wrap($btn).should('have.value', '111111');
    });
  ...
  it('Interação com botões de PopUps e Ok automatico', () => {
    cy.visit('https://www.wcaquino.me/cypress/componentes.html')
    cy.get('#alert').click()
